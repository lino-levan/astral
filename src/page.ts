...
export class Page extends EventTarget {
  ...
  async #go(
      ...
      const history = await retryDeadline(this.#celestial.Page.getNavigationHistory(), this.timeout);
      const entry = history.entries[history.currentIndex + delta];
      if (!entry) {
        throw new Error("Tried to navigate to history entry that does not exist.");
      }
